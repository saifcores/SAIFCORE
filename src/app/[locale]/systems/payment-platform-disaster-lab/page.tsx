...
import { CaseStudyPaymentDisasterLab } from "@/components/portfolio/CaseStudyPaymentDisasterLab";
...
import { PAYMENT_DISASTER_LAB_CASE_STUDY_PATH } from "@/data/case-studies";
...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    ...
    namespace: "caseStudyPaymentDisasterLab",
  });
  return buildPageMetadata({
    locale,
    path: PAYMENT_DISASTER_LAB_CASE_STUDY_PATH,
    title: t("metaTitle"),
    description: t("metaDescription"),
  });
}

export default async function PaymentDisasterLabCaseStudyPage({
  params,
}: Props) {
  const { locale } = await params;
  setRequestLocale(locale as Locale);
  const t = await getTranslations("caseStudyPaymentDisasterLab");
  const tCommon = await getTranslations("common");
  const tSystems = await getTranslations("systemsPage");
  const messages = await getMessages();
  const stacks = [...messages.caseStudyPaymentDisasterLab.stacks];
  ...
  const pageJsonLd = buildCaseStudyPageGraph({
    ...
    externalUrl: "https://github.com/saifcores/payment-platform-disaster-lab",
    keywords: stacks,
    breadcrumb: [
      { name: tCommon("home"), path: "/" },
      { name: tSystems("title"), path: "/systems" },
      { name: t("title"), path: PAYMENT_DISASTER_LAB_CASE_STUDY_PATH },
    ],
  });
