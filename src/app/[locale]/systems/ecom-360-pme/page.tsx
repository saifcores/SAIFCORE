...
import { CaseStudyEcom360 } from "@/components/portfolio/CaseStudyEcom360";
...
import { ECOM_360_CASE_STUDY_PATH } from "@/data/case-studies";
...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    ...
    namespace: "caseStudyEcom360",
  });
  return buildPageMetadata({
    locale,
    path: ECOM_360_CASE_STUDY_PATH,
    title: t("metaTitle"),
    description: t("metaDescription"),
  });
}

export default async function Ecom360CaseStudyPage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale as Locale);
  const t = await getTranslations("caseStudyEcom360");
  const tCommon = await getTranslations("common");
  const tSystems = await getTranslations("systemsPage");
  const messages = await getMessages();
  const stacks = [...messages.caseStudyEcom360.stacks];
  ...
  const pageJsonLd = buildCaseStudyPageGraph({
    ...
    externalUrl: "https://ecom360pme.com/",
    keywords: stacks,
    breadcrumb: [
      { name: tCommon("home"), path: "/" },
      { name: tSystems("title"), path: "/systems" },
      { name: t("title"), path: ECOM_360_CASE_STUDY_PATH },
    ],
  });
